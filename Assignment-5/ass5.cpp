...
void findShortestPath(int numNodes, const vector<vector<int>>& adjMatrix) {
    ...
    if (minCost[0] == INF) {
        cout << "No valid route exists from the source to the destination." << endl;
    } else {
...
int main() {
    ...
    cout << "\nEnter " << edgeCount << " routes (format: source destination cost):" << endl;
    for (int i = 0; i < edgeCount; ++i) {
        ...
        if (source >= 0 && source < nodeCount && dest >= 0 && dest < nodeCount) {
            ...
            cout << "Warning: Invalid node number entered. Skipping this route." << endl;
        }
    }
