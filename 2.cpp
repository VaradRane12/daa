...
vector<Movie> generateMovies(int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        m.popularity = rand() % 1'000'000 + 1'000;               // 1,000 to 1,000,000
        movies.push_back(m);
    }
    return movies;
}
...
int main() {
    ...
    int startIdx = max(0, num_movies - 10);
    for (int i = num_movies - 1; i >= startIdx; --i) {
        movies[i].display();
    }
